...
$gurukul_password = mysqli_real_escape_string($con, md5($_POST['password']));

// $vartest =md5($_POST['password']);
if( (isset($_POST["gurukul_admin"])) && (isset($_POST["gurukul_password"]))  )
{	
// $result = mysqli_query($con,"SELECT * FROM users WHERE email='" . $gurukul_admin. "' and password = '". $gurukul_password."'");
$result = mysqli_query($con,"SELECT * FROM users WHERE email='$gurukul_admin' and password = '$gurukul_password'");
